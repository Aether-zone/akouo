...
RUN pnpm --filter @akouo/api --filter @akouo/app build

# The api's webpack bundle leaves everything under node_modules external, so it
# needs a real installed tree at runtime. `pnpm deploy` resolves the api's
# production dependencies out of the workspace lockfile and writes a
# self-contained directory, node_modules and all.
